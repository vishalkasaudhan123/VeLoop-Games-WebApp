...
// const rewards=[
//   {id:"ve",name:"VE",cost:50},
//   {id:"sve",name:"SVE",cost:100},
//   {id:"gems",name:"Gems",cost:150},
//   {id:"tokens",name:"Tokens",cost:200},
//   {id:"spins",name:"Spins",cost:250}
// ];
...
//   const redeem=(r)=>{
//     if(!redeemGameCoins(r.cost)){setInsufficient(true);return;}
//     if(r.id==="tokens") addTokens(20);
//     setMessage(`${r.name} redemption confirmed for ${r.cost} Game Coins.`);
//   };
//   return <div><WalletBar/><main className={styles.main}><div className={styles.head}><div><span>REWARDS</span><h1>Redeem Game Coins</h1></div><div className={styles.balance}><Coins/> {gameCoins}</div></div>
//     <div className={styles.grid}>{rewards.map(r=><article key={r.id}><Gift size={30}/><h2>{r.name}</h2><p>{r.cost} Game Coins</p><button onClick={()=>redeem(r)}>Redeem</button></article>)}</div>
//     {message&&<div className={styles.success}>{message}</div>}<Link className={styles.back} to="/games">← Back to Games</Link>
//     {insufficient&&<InsufficientModal type="coins" onClose={()=>setInsufficient(false)}/>}
//   </main></div>;
// }
